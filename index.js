...
// ✅ New API endpoint to update the owner's profile
app.post('/update-profile', (req, res) => {
    const { name, age, relation, from, study, fbLink, whatsappNo, dpLink } = req.body;

    // A simple validation to ensure all fields are present
    if (!name || !age || !relation || !from || !study || !fbLink || !whatsappNo || !dpLink) {
        return res.status(400).json({ error: "सभी प्रोफ़ाइल फ़ील्ड आवश्यक हैं।" });
    }

    try {
        const profileData = { name, age, relation, from, study, fbLink, whatsappNo, dpLink };
