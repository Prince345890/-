...
// ✅ Corrected API endpoint to update the owner's profile
app.post('/update-profile', (req, res) => {
    // gfName ko yahan include kiya gaya hai
    const { name, age, relation, from, study, fbLink, whatsappNo, dpLink, gfName } = req.body;

    // Validation to ensure all fields are present
    if (!name || !age || !relation || !from || !study || !fbLink || !whatsappNo || !dpLink || !gfName) {
        return res.status(400).json({ error: "सभी प्रोफ़ाइल फ़ील्ड आवश्यक हैं।" });
    }

    try {
        // profileData object mein gfName ko bhi add kiya gaya hai
        const profileData = { name, age, relation, from, study, fbLink, whatsappNo, dpLink, gfName };
        fs.writeFileSync(path.join(__dirname, 'ownerProfile.json'), JSON.stringify(profileData, null, 4), 'utf-8');
        res.status(200).json({ message: "प्रोफ़ाइल सफलतापूर्वक अपडेट हो गई है।" });
    } catch (error) {
        logger(`Failed to update profile: ${error.message}`, "[ ERROR ]");
        res.status(500).json({ error: "प्रोफ़ाइल अपडेट करने में असमर्थ।" });
    }
});
