...
try {
    ...
    return logger.loader("Appstate file not found!", "error")
}
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
        ...
        global.client.timeStart = new Date().getTime(),
            function() {
                // Yahan `Priyansh` folder ka naam `prince` kar diya gaya hai
            ...
            function() {
                // Yahan bhi `Priyansh` folder ka naam `prince` kar diya gaya hai
        ...
        try {
            const ownerProfile = JSON.parse(readFileSync('ownerProfile.json', 'utf-8'));
            const welcomeMsg = `Greetings, ${ownerProfile.name}! Your bot, **${global.config.BOTNAME}**, has been successfully activated.`;
            loginApiData.sendMessage(welcomeMsg, global.config.ADMINBOT[0]);
        } catch (e) {
            console.log("Failed to send welcome message to admin: " + e.message);
        }
        ...
        require('./includes/listenCustom')({ api: loginApiData });
